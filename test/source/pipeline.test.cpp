...
TEST_CASE("pipeline metrics")
{
    ...
    auto metrics = pipeline.getMetricsAndReset();
    REQUIRE(metrics.size() == 3);
    for (auto& metric : metrics) {
        REQUIRE(metric.counter == 50);
        REQUIRE(metric.wallTime > std::chrono::duration<double>::zero());
    }

    // after reset
    metrics = pipeline.getMetricsAndReset();
    REQUIRE(metrics.size() == 3);
    for (auto& metric : metrics) {
        REQUIRE(metric.counter == 0);
        REQUIRE(metric.wallTime == std::chrono::duration<double>::zero());
    }
}

TEST_CASE("pipeline metrics reset")
{
    auto pipeline = TestFilter1() | TestFilter2() | TestFilter3();
